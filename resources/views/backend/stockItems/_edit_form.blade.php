...
<div class="form-group {{ $errors->has('is_active') ? 'has-error' : '' }}">
    ...
    <div class="col-md-8">
        <div class="cm-switch">
            {{ Form::radio(fake_field('is_active'), ACTIVE_STATUS_ACTIVE, ($stockItem->is_active == ACTIVE_STATUS_ACTIVE), ['id' => fake_field('is_active') . '-active', 'class' => 'cm-switch-input', 'data-cval-name' => 'The active status field', 'data-cval-rules' => 'integer|in:' . array_to_string(active_status())]) }}
            <label for="{{ fake_field('is_active') }}-active" class="cm-switch-label">{{ __('Active') }}</label>

            {{ Form::radio(fake_field('is_active'), ACTIVE_STATUS_INACTIVE, ($stockItem->is_active == ACTIVE_STATUS_INACTIVE), ['id' => fake_field('is_active') . '-inactive', 'class' => 'cm-switch-input']) }}
...
<div class="form-group {{ $errors->has('is_ico') ? 'has-error' : '' }}">
    ...
    <div class="col-md-8">
        <div class="cm-switch">
            {{ Form::radio(fake_field('is_ico'), ACTIVE_STATUS_ACTIVE, ($stockItem->is_ico == ACTIVE_STATUS_ACTIVE), ['id' => fake_field('is_ico') . '-yes', 'class' => 'cm-switch-input', 'data-cval-name' => 'The ICO field', 'data-cval-rules' => 'integer|in:' . array_to_string(active_status()), 'v-model' => 'hideIcoOptionFields']) }}
            <label for="{{ fake_field('is_ico') }}-yes" class="cm-switch-label">{{ __('Yes') }}</label>

            {{ Form::radio(fake_field('is_ico'), ACTIVE_STATUS_INACTIVE, ($stockItem->is_ico == ACTIVE_STATUS_INACTIVE), ['id' => fake_field('is_ico') . '-no', 'class' => 'cm-switch-input', 'v-model' => 'hideIcoOptionFields']) }}
...
<div v-if="hideIcoOptionFields == 0">
    ...
    <div class="form-group {{ $errors->has('exchange_status') ? 'has-error' : '' }}">
        ...
        <div class="col-md-8">
            <div class="cm-switch">
                {{ Form::radio(fake_field('exchange_status'), ACTIVE_STATUS_ACTIVE, ($stockItem->exchange_status == ACTIVE_STATUS_ACTIVE), ['id' => fake_field('exchange_status') . '-active', 'class' => 'cm-switch-input', 'data-cval-name' => 'The exchange status field', 'data-cval-rules' => 'integer|in:' . array_to_string(active_status())]) }}
                <label for="{{ fake_field('exchange_status') }}-active" class="cm-switch-label">{{ __('Active') }}</label>

                {{ Form::radio(fake_field('exchange_status'), ACTIVE_STATUS_INACTIVE, ($stockItem->exchange_status == ACTIVE_STATUS_INACTIVE), ['id' => fake_field('exchange_status') . '-inactive', 'class' => 'cm-switch-input']) }}
    ...
    {{--        <div class="col-md-8">--}}
    {{--            <div class="cm-switch">--}}
    {{--                {{ Form::radio(fake_field('is_fee_applicable'), ACTIVE_STATUS_ACTIVE, ($stockItem->is_fee_applicable == ACTIVE_STATUS_ACTIVE), ['id' => fake_field('is_fee_applicable') . '-yes', 'class' => 'cm-switch-input', 'data-cval-name' => 'The fee applicable field', 'data-cval-rules' => 'integer|in:' . array_to_string(active_status())]) }}--}}
    {{--                <label for="{{ fake_field('is_fee_applicable') }}-yes" class="cm-switch-label">{{ __('Yes') }}</label>--}}

    {{--                {{ Form::radio(fake_field('is_fee_applicable'), ACTIVE_STATUS_INACTIVE, ($stockItem->is_fee_applicable == ACTIVE_STATUS_INACTIVE), ['id' => fake_field('is_fee_applicable') . '-no', 'class' => 'cm-switch-input']) }}--}}
    ...
    <div v-if="showOptionalFields">
        ...
        <div class="form-group {{ $errors->has('deposit_status') ? 'has-error' : '' }}">
            ...
            <div class="col-md-8">
                <div class="cm-switch">
                    {{ Form::radio(fake_field('deposit_status'), ACTIVE_STATUS_ACTIVE, ($stockItem->deposit_status == ACTIVE_STATUS_ACTIVE), ['id' => fake_field('deposit_status') . '-active', 'class' => 'cm-switch-input', 'data-cval-name' => 'The deposit status field', 'data-cval-rules' => 'integer|in:' . array_to_string(active_status())]) }}
                    <label for="{{ fake_field('deposit_status') }}-active" class="cm-switch-label">{{ __('Active') }}</label>

                    {{ Form::radio(fake_field('deposit_status'), ACTIVE_STATUS_INACTIVE, ($stockItem->deposit_status == ACTIVE_STATUS_INACTIVE), ['id' => fake_field('deposit_status') . '-inactive', 'class' => 'cm-switch-input']) }}
        ...
        <div class="form-group {{ $errors->has('withdrawal_status') ? 'has-error' : '' }}">
            ...
            <div class="col-md-8">
                <div class="cm-switch">
                    {{ Form::radio(fake_field('withdrawal_status'), ACTIVE_STATUS_ACTIVE, ($stockItem->withdrawal_status == ACTIVE_STATUS_ACTIVE), ['id' => fake_field('withdrawal_status') . '-active', 'class' => 'cm-switch-input', 'data-cval-name' => 'The withdrawal status field', 'data-cval-rules' => 'integer|in:' . array_to_string(active_status())]) }}
                    <label for="{{ fake_field('withdrawal_status') }}-active" class="cm-switch-label">{{ __('Active') }}</label>

                    {{ Form::radio(fake_field('withdrawal_status'), ACTIVE_STATUS_INACTIVE, ($stockItem->withdrawal_status == ACTIVE_STATUS_INACTIVE), ['id' => fake_field('withdrawal_status') . '-inactive', 'class' => 'cm-switch-input']) }}
            ...
            <div class="col-md-8">
                <select class="form-control" id="api-services" data-cval-name="{{ __('The API service field') }}" data-cval-rules="require" name="{{ fake_field('api_service') }}">
                    ...
                    <option
                            ...
                            v-bind:value="index"
                            v-text="api"
                            @if(!is_null($stockItem->api_service))
                            :selected="index == {{$stockItem->api_service}}"
                            @endif
                    ></option>
                </select>
